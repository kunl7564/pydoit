...
def getlogrange(defaultrange='HEAD', end_only=True):
        ...
        if len(conf['start_date']) > 0 and len(conf['end_date']) > 0 :
                return '--since=%s --until=%s %s' % (conf['start_date'], conf['end_date'], commit_range)
...
def getgnuplotversion():
	return "gnu"  
	# kunl getpipeoutput(['%s --version' % gnuplot_cmd]).split('\n')[0]
...
class GitDataCollector(DataCollector):
	def collect(self, dir):
		...
		self.total_authors += int(getpipeoutput(['git shortlog -s %s --branches=*' % getlogrange(), 'wc -l']))
		...
		for line in lines:
			...
			output = getpipeoutput(['git log "%s" --pretty=format:"%%at %%aN" -n 1 --branches=*' % hash])
			if len(output) > 0:
				parts = output.split(' ')
				stamp = 0
				try:
		...
		for tag in reversed(tags_sorted_by_date_desc):
			cmd = 'git shortlog -s "%s" --branches=*' % tag
			if prev != None:
				cmd += ' "^%s"' % prev
		...
		lines = getpipeoutput(['git rev-list --pretty=format:"%%at %%ai %%aN <%%aE>" %s --branches=*' % getlogrange('HEAD'), 'grep -v ^commit']).split('\n')
		if len(lines) < 4:
			return;
		for line in lines:  # 1519872790 2018-03-01 10:53:10 +0800 shroom <guomiao@tpson.cn>
			parts = line.split(' ', 4)
			if len(parts) < 4:
				break;
			author = ''
		...
		revlines = getpipeoutput(['git rev-list --pretty=format:"%%at %%T" %s --branches=*' % getlogrange('HEAD'), 'grep -v ^commit']).strip().split('\n')
		lines = []
		revs_to_read = []
		time_rev_count = []
		...
		for line in lines:  # 1519872553 1128
			parts = line.split(' ')
			if len(parts) != 2:
				continue
		...
		lines = getpipeoutput(['git log --shortstat %s --pretty=format:"%%at %%aN" %s --branches=*' % (extra, getlogrange('HEAD'))]).split('\n')
		lines.reverse()
		...
		lines = getpipeoutput(['git log --shortstat --date-order --pretty=format:"%%at %%aN" %s --branches=*' % (getlogrange('HEAD'))]).split('\n')
		lines.reverse()
	...
	def revToDate(self, rev):
		stamp = int(getpipeoutput(['git log --pretty=format:%%at "%s" -n 1 --branches=*' % rev]))
...
if __name__ == '__main__':
	g = GitStats()
	args = ['-c', 'start_date=2018-3-1', '-c', 'end_date=2018-3-29', 'E:\\WebProjects\\electrical_safety', 'e:\\tmp']
	g.run(args)
	# g.run(sys.argv[1:])
